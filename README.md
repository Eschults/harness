...
2. **Create the implementation routine.** At [claude.ai/code/routines](https://claude.ai/code/routines), create a routine pointed at this repository, using the short prompt in the header comment of `claude-issue-to-pr.yml`. Then edit it, add an **API** trigger, and click **Generate token** — the token is shown once. Save it as the `CLAUDE_ROUTINE_TOKEN` repo secret and the routine id (`trig_...`) as the `CLAUDE_ROUTINE_ID` repo variable. Pick the routine's cloud environment deliberately: its network allowlist, environment variables, and connector list are the sandbox the agent runs in, and every connector you leave attached is a tool the agent can write with, unprompted.

3. **Create the triage routine**, the same way, using the prompt in the header comment of `sentry-triage.yml`. Store its token and id as `CLAUDE_TRIAGE_ROUTINE_TOKEN` and `CLAUDE_TRIAGE_ROUTINE_ID`. Two things matter here: pick a cheap model in the routine's model selector, since triage is a yes/no call rather than a coding task; and attach your GitHub connector and nothing else, because the label it applies is the only output this routine has and any other connector is reach it does not need.
...
- **Draft PRs only, never auto-merge.** A human always reviews before anything ships. This is enforced in `.claude/prompts/issue-to-pr.md` and in `CLAUDE.md`, not just assumed.
- **Triage before spend.** The Sentry path runs a cheap, read-only pass on a small model before ever letting the agent touch code, so a wave of low-value errors doesn't turn into a wave of low-value PRs.
- **Least reach per routine.** Cloud sessions have no `--max-turns` or `--allowedTools`. The equivalent controls are per-routine and set in the routine itself: the model, the cloud environment's network allowlist, the list of attached connectors, and the repositories it can clone. The triage routine gets the GitHub connector and nothing else.
- **Untrusted-input handling.** Issue titles and bodies are written by Sentry and Notion, so `fire-claude-routine` passes them to the API through environment variables rather than `${{ }}` interpolation inside a shell script, and the routine receives them inside a `<routine-fire-payload>` block labeled as data rather than instructions. Both prompts in `.claude/prompts/` say explicitly what to do with instructions found inside that block.
- **Concurrency groups** keyed on issue number stop the same issue from spawning parallel dispatches if it gets re-labeled. Note this only guards the dispatch — two sessions fired seconds apart are two independent cloud sessions with nothing coordinating them.
- **Branch protection at the source.** Cloud sessions push to `claude/`-prefixed branches; a push to any other branch is rejected if it is protected, has someone else's open PR, or carries commits by another author.
- **Explicit no-go list** in `CLAUDE.md` (auth, payments, migrations, CI config) that the agent is instructed to bounce to a human instead of guessing on.
- **Every dispatch is announced on the issue**, on success with the session URL and on failure with the Actions run URL, so a fired-and-lost session leaves a trace. A failed triage dispatch also labels the issue `needs-human`, since nothing downstream would otherwise pick it up.
